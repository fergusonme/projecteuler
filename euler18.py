...
def sum_rows(tri, row):
  for i in range(len(tri[row])):
    tri[row][i] += max(tri[row+1][i],tri[row+1][i+1])
  if row == 1:
    tri[0] += max(tri[row][0],tri[row][1])
    return tri[0]
  else:
    return sum_rows(tri,row-1)   

start = time.time()
answer = sum_rows(triangle, len(triangle)-2)
